    ...
    public class SideDPK
    {
        public static void ExtractDPK(string DpkFile)
        {
            try
            {
                ...
                using (FileStream DpkStream = new FileStream(DpkFile, FileMode.Open, FileAccess.Read))
                {
                    using (BinaryReader DpkReader = new BinaryReader(DpkStream))
                    {
                        ...
                        for (int f = 0; f < Entries; f++)
                        {
                            ...
                            using (FileStream SplitFileOut = new FileStream(Extract_dir + "/" + fname + $"{FileCount}",
                ...
                CoreForm.AppMsgBox("Extracted " + Path.GetFileName(DpkFile) + " file", "Success", MessageBoxIcon.Information);
            }
            catch (Exception ex)
            {
